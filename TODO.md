...
## Re-base the golden-trace corpus on the C++ engine — DONE 2026-07-12
Replaced by the C++ self-regression manifest gate (user decisions: manifest form with full
final state, 1M games, mixed ai/random policy, 2k committed subset). The 1000 random
Python-parity traces, the 100k extended corpus, and the 2 quarantined traces were retired;
the 28 hand-authored scenario traces remain on the forced-replay path. See
`SCRIPTS/record_cpp_manifest.py`, `tests/test_cpp_manifest_gate.py`, and record
`cpp_manifest_gate`.
