...
- [wave_unlock.py](wave_unlock.py) — Computes measured wave-unlock counts (matchups whose scope mask ⊆ candidate bit-set) from `audit_analytic ... --combo-top 10000` output on stdin. `--waves` overrides the default candidate ladder; fails loud on truncated combo lists.
